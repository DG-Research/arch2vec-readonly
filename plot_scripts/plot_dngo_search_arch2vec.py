...
def plot_over_time_dngo_search_arch2vec(name, cmap=plt.get_cmap("tab10")):
    ...
    ax.plot(time_plot, valid_plot, color=cmap(6), lw=2, label='{}: {}'.format('arch2vec', 'BO'))
    ax_test.plot(time_plot, test_plot, '--', color=cmap(6), lw=2, label='{}: {}'.format('arch2vec', 'BO'))
    ax.set_xscale('log')
    ax.set_yscale('log')
    ...
    with open('results/{}-{}-nasbench-101.json'.format('BO', name), 'w') as f_w:
...
if __name__ == '__main__':
    name = 'arch2vec'
    plot_over_time_dngo_search_arch2vec(name)
